...
// Glass with custom corner radius
VStack { content }
...
**Glass styles:**
- `.regular` — Standard glass for cards and containers
- `.regular.interactive()` — For tappable elements (responds to press states)

**Benefits of `.glassEffect()`:**
...
5. **Future-proof** — Apple can update the effect without code changes

We'll use `.glassEffect()` extensively in Lessons 2.2 and 2.3.
...
**Note**: With iOS 26's `.glassEffect()`, you typically don't need manual shadows for glass elements — the system handles depth and elevation automatically. These shadow tokens are useful for non-glass elements.
...
- **Colors**: Category colors, semantic colors, feedback colors
- **Glass**: Semantic configuration for iOS 26's native `.glassEffect()` modifier
...
- **Shadows**: Elevation levels for non-glass elements

This foundation will be used by every component you build in Phase 2. The Glass configuration works with iOS 26's native Liquid Glass system for consistent, performant glass effects.
...
view.glassEffect()                                                     // Default glass
view.glassEffect(.regular, in: .rect(cornerRadius: 16))                // Glass card
view.glassEffect(.regular.interactive(), in: .rect(cornerRadius: 12))  // Glass button
...
In **Lesson 2.2: View Modifiers Deep Dive**, you'll learn how to use iOS 26's native `.glassEffect()` modifier and create custom view modifiers that apply these tokens automatically, like `.glassCard()` and `.categoryAccent()`.
